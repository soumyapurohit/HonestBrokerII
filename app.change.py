...
class CreateRequestForm(FlaskForm):
    ...
    cur.execute('SELECT * FROM data_catalog')
    resulset = cur.fetchall()
    datachoices = resulset[1]

    datasetname = SelectField(choices = [datachoices])
...
@login_required
def dashboard():
    #datasetname = 'unique patient id taking drug'
   ...
   # cur.execute('SELECT * FROM data_catalog where "dataset_name" = %s', [datasetname])
    ...
    if(current_user.username == 'Admin'):
        ...
        return render_template('dashboard.html', name = current_user.username, apprInternal_info= apprInternal_info, request_info=request_info, deniedInternal_info = deniedInternal_info)
...
def submitrequest():
     ...
     datasetprint=form.datasetname.data.datasetid
     print(type(datasetprint))
     print(datasetprint)
    ...
    #determining input_risk for data_type
     if(form.typeofdata.data == 'identified'):
         ...
         input_risk[1] = 3;
     dataset_risk = form.datasetname.data.dataset_risk;
     accept_risk = form.datasetname.data.accept_risk;
     ...
     request_info = RequestForm.query.filter_by(ownerid=current_user.id, status = 'pending').all()
     apprInternal_info = RequestForm.query.filter_by(status = 'approved').all()
     deniedInternal_info = RequestForm.query.filter_by(status= 'denied').all() 
